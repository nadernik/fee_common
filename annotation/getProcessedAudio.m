function [audio, absTime, syllType, dura] = getProcessedAudio(birdName, varargin)
	...
	dPitch = getProcessedDataFiles(birdName, 'dataType', 'audio', 'experNames', P.experNames, 'prefix', P.prefix, 'rootdir', P.rootdir);
	...
	for nFile = 1:length(dMisc)
		if(sum(bSel{nFile})>0)       
			...
			audio(currNdx+1:currNdx+sum(bSel{nFile})) = {rawaudio.segs(bSel{nFile}).audio};
	...
	if(~isempty(P.targetRegion))
		if strcmp(P.targetMethod,'percent')
			pitchTraj = cellfun(@extractFragment_percent, audio, repmat({P.targetRegion},length(audio),1), 'UniformOutput', false);
