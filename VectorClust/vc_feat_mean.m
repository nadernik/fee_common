function vcdb = vc_feat_mean(vcdb, vf_name, varargin)
P.sfname = '';
P.time_range = [];
P.percent_range = [];
P.samples_range = [];
...
if isempty(P.sfname)
    sfname = ['mean_' vf_name]; % may add to this later base on range
...
elseif ~isempty(P.percent_range)
    for syll = 1:length(vcdb.d.v)
        ...
        vcdb.d.sf(syll, sf_idx) = mean(vcdb.d.vf{vf_idx}{syll}(ndxStart:ndxEnd));
    end
    if use_default_sfname
        % if user did not supply a name for this feature, add the time
        % range to our default name so it becomes like "mean_pitch_10_20".
...
elseif ~isempty(P.samples_range)
    for syll = 1:length(vcdb.d.v) % for each syllable
        vcdb.d.sf(syll, sf_idx) = mean(vcdb.d.vf{vf_idx}{syll}(P.samples_range(1):P.samples_range(2)));
    end
    if use_default_sfname
    ...
    % vector
    vcdb.d.sf(:,sf_idx) = cellfun(@mean, vcdb.d.vf{vf_idx});
end
