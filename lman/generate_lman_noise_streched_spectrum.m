function final_noise = generate_lman_noise_stretched_spectrum(rows, cols, stretch_factor, debugging)
...
tail_start = 35; % point in filter spectrum that starts the tail
tail_end = 100;
...
P = f(1:(nfft/2+1));
ac_filter_power = sum(P(2:end));
% Fit a line to the tail of the log spectrum 
tail = P(tail_start:tail_end);
x = tail_start:tail_end;
coefs = polyfit(log(x'),log(tail),1);
% Use the fit to extend the tail as necessary
len = round((nfft/2+1)/ stretch_factor); % length we need to start with to get nfft points after stretching
if len > tail_start
    x = tail_start:len;
    tail_from_fit = exp(coefs(1) * log(x) + coefs(2));
    filter_extended = [P(1:tail_start-1); tail_from_fit'];
else
    filter_extended = frequency_domain_filter(1:len);
end
half_filter_stretched = interp1(1:len, filter_extended, linspace(1,len, nfft/2+1));% Rescale the spectrum

% Rescale amplitude of filter so that it has the same total power as it did
% before we stretched it.
%half_filter_stretched(2:end) = half_filter_stretched(2:end) ./ sum(half_filter_stretched(2:end)) .* ac_filter_power;


frequency_domain_filter = [half_filter_stretched, half_filter_stretched(end-1:-1:2)]' * ones(1, cols);
...
final_noise = final_noise ./ std(final_noise(:)) .* 1.7026;
