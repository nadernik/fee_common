...
% pitchGUI M-file for pitchGUI.fig
%      pitchGUI, by itself, creates a new pitchGUI or raises the existing
%      singleton*.
%
%      H = pitchGUI returns the handle to a new pitchGUI or the handle to
%      the existing singleton*.
%
%      pitchGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in pitchGUI.M with the given input arguments.
%
%      pitchGUI('Property','Value',...) creates a new pitchGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before pitchGUI_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to pitchGUI_OpeningFcn via varargin.
%
...
% if we were passed an exper, load it
if nargin > 3
...
%     cd(handles.rootdir) % going up exper and bird directory
    ...
    handles.exper = exper;
    % try to load filter paramters
    try
        p = rulesHandles.rules(rulesHandles.rSel).params;
        set(handles.textPitchTarget,'String',num2str( p.pitchTarget))
        set(handles.textNudge,'String',num2str( p.nudge))
        set(handles.textWidth,'String',num2str( p.width))
        set(handles.textOverlap,'String',num2str( p.overlap))
        set(handles.textPitchThreshold,'String',num2str( p.pitchThreshold))
        set(handles.textBottomFreq,'String',num2str( p.bottomFreq))
        set(handles.textPassIn,'String',num2str( p.passIn))
        set(handles.textDbDown,'String',num2str( p.dBdown))
        set(handles.popupHarmonics,'Value', p.harmonics)
        set(handles.textBandFilter,'String',num2str( p.lpfCutoff))
        set(handles.textDbDownBandFilter,'String',num2str( p.lpfdBdown))
        if strcmp(p.push,'up')
            set(handles.uipanel1,'SelectedObject',handles.buttonUp)
        else
        ...
        e = lasterror;
        if strcmp(e.identifier,'MATLAB:nonExistentField') || strcmp(e.identifier, 'MATLAB:nonStrucReference')
...
handles.bandFilters = makeFilter('bUp',handles.bUp,'pitchTarget',handles.pitchTarget,'nudge',handles.nudge,'width',handles.width,...
    ...
    e = lasterror;
    idx = strfind(e.message,10); %look for newline
    str = e.message(idx(end)+1:end); % take only the part after the last newline
    warndlg(str,'Error Making Filters','modal')
    return
end
...
%DefaultName = ['pitchgui_',handles.birdName,'_',datestr(date,29),'.mat']; %
