function [handles, didSegment] = BigBatchAutoSegmentor(handles)
    % ElectroGui macro
    ...
    fls = 1:length(handles.sound_files);
    ...
    didSegment = false;
    if ~isempty(dir('*.dat')) || ~isempty(dir('*.wav'))
        for j = 1:length(fls) % counter for files to be segmented
            c = fls(j); % file number

            [snd, fs, dt, ~, ~] = eval(['egl_' handles.sound_loader '([''' handles.path_name filesep handles.sound_files(c).name '''],1)']);
