...
if exist('regen', 'var') && regen == true
    template = 3*sin(linspace(0, 4*pi, T)) + 4;
    meansong = zeros(size(template)) + 4;
...
plot(mean(songs, 2), 'Color', colors.mean, 'LineWidth', 3)
x = t + [-0.5, 0.5];
y = interp1(1:T, songs(:, nred), x);
plot(x, y, 'Color', colors.bad, 'LineWidth', 3)
y = interp1(1:T, songs(:, ngreen), x);
plot(x, y, 'g', 'Color', colors.good, 'LineWidth', 3)
plot(template, 'Color', colors.template, 'LineWidth', 3)
