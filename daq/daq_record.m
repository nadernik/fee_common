...
if all(~BTRIGGER(matchannels))
    ...
    for chanNo = 1:nChan
        if ~BTRIGGER(matchannels(chanNo))
            TRIGGERSTART(matchannels(chanNo)) = startSamp;
            TRIGGEREND(matchannels(chanNo)) = endSamp;
            filenames{chanNo} = [filename, 'chan', num2str(channels(chanNo)), '.dat'];
            TRIGGERFILENAME{matchannels(chanNo)} = [filename, 'chan', num2str(channels(chanNo)), '.dat'];
            BTRIGGER(matchannels(chanNo)) = true;
            bStatus(chanNo) = true;
        else
