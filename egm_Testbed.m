function handles = egm_bigBatchSegment_elm(handles)
% batch segments all files, then moves on to the next day
% get info
[pathstr, day, ext] = fileparts(handles.dbase.PathName);
[pathstr, bird, ext] = fileparts(pathstr);
handles.AutomationELM = []; 

% segment files
handles = egm_Batch_segment(handles); 

% save segmentation
dbase = GetDBase(handles);
save(fullfile(pathstr,bird,day, 'analysisAuto.mat'),'dbase');

% move on to next day, if there is one
try 
    day = datestr(datenum(day)+1, 'yyyy-mm-dd');
    handles.AutomationELM.path = fullfile(pathstr, bird, day); 
    [handles] = loadNew(handles); 
    handles.dbase = GetDBase(handles);
    display(['analyzing bird ', num2str(bird), ' day ', day])
    handles = egm_Testbed(handles); 
catch exception
    display(exception); 
    day = datestr(datenum(day)+1, 'yyyy-mm-dd');
    if datenum(day)>datenum(now)
    else
        handles.AutomationELM.path = fullfile(pathstr, bird, day); 
        [handles] = loadNew(handles); 
        handles.dbase = GetDBase(handles);
...
handles = rmfield(handles, 'AutomationELM')



%% functions...
%% load new day
function handles = loadNew(handles)
handles.IsUpdating = 0;
[handles ischanged] = eg_NewExperimentAutoELM(handles);
if ischanged == 0
    return
end
