...
% load('C:\Users\emackev\Dropbox (MIT)\TempFileTransfer\RawForPengcheng')
% Ysiz = size(Y); 
% imwrite(Y(:,:,1), 'C:\Users\emackev\Dropbox (MIT)\TempFileTransfer\RawForPengchengTif.tif')
% for k = 3:size(Y,3)
%     written = 0;
%     while written == 0; 
%         try
%             imwrite(Y(:,:,k), 'C:\Users\emackev\Dropbox (MIT)\TempFileTransfer\RawForPengchengTif.tif', 'writemode', 'append');
%             written = 1; 
%             pause(.1)
%         catch
%             pause(1); 
