...
nIterPlotSplit1 = 700;  % number of splitting iterations before plotting intermediate splitting phase
...
PsylInput(:,mod(1:p.nsteps,p.trainint)==1) = 1; HowOn; % rhythmic activation of training neurons
...
plotHVCnet(wProto,xdynProto,p.trainint,trainingNeuronsAlt,PlottingParams)
set(gca, 'color', 'none');
PlottingParams.axesPosition = [PlottingParams.thisPanel/nplots-2*spacing, bottom, 40*scale, height];
plotAlternating(wProto, xdynProto, p.trainint, trainingNeuronsAlt, PlottingParams)
set(gca, 'color', 'none')
