function Latency = findHVClatency_boutOnset(xsort, trainingNeurons)
...
for syli = 1:length(trainingNeurons)
    ...
    for ni = 1:n
        ...
        Latency{syli}.FireDur(ni) = max(nFired(ni,:))>thres1*(syli==1)+thres2*(syli==2); %.25*length(trainingNeurons{syli}.tind);  % count if it fired at that latency in more than 1/4 of the syllables
    end
end
