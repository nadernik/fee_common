...
rectangle('Position', [rx ry rw rh], ...
    'FaceColor',  'none', ...
    'LineStyle', '-', 'LineWidth', 1, 'EdgeColor', PlottingParams.SeedColor,...
    'curvature', [.98 .1])
...
subplot('position', [ploti/nplots-2*spacing, bottom, length(tindplot1)*scale, height])
...
if isfield(PlottingParams,'boutOnsetElement')
    ...
    if PlottingParams.thisPanel>1 
        text((-10+trainingNeurons{1}.candLat(1)*10-10)/2,-10,'\epsilon','fontsize',7)
        text(((-10+trainingNeurons{2}.candLat(1)*10)+(trainingNeurons{2}.candLat(end)*10))/2,-10,'\alpha','fontsize',7)
    end
else
    patch([-10+trainingNeurons{1}.candLat(1)*10 trainingNeurons{1}.candLat(end)*10 trainingNeurons{1}.candLat(end)*10 -10+trainingNeurons{1}.candLat(1)*10],[-4 -4 -2 -2],PlottingParams.Syl1BarColor);
    if PlottingParams.thisPanel>2
        text(((-10+trainingNeurons{1}.candLat(1)*10)+(trainingNeurons{1}.candLat(end)*10))/2,-10,'\beta','fontsize',7)
    end
end
ylim([-5 ntot])
...
% Plot protosyllable.
subplot('position', [ploti/nplots-spacing, bottom, length(tindplot2)*scale, height])
...
patch([-10+trainingNeurons{2}.candLat(1)*10 trainingNeurons{2}.candLat(end)*10 trainingNeurons{2}.candLat(end)*10 -10+trainingNeurons{2}.candLat(1)*10],[-4 -4 -2 -2],PlottingParams.Syl2BarColor);
if isfield(PlottingParams,'boutOnsetElement')
    if PlottingParams.thisPanel>1
        text(((-10+trainingNeurons{2}.candLat(1)*10)+(trainingNeurons{2}.candLat(end)*10))/2,-10,'\alpha','fontsize',7)
    end
