...
% Emily Mackevicius 1/21/2015, heavily copied from Hannah Payne's code
...
for t = 1:(nIterProto+nIterPlotSplit2)  
    ...
    bdyn(1:k,:) = protosyllableStage(t)*PsylInput+splittingStage(t)*AltInput; % drive seed neurons
    p.input = bdyn;
    % run one iteration
    [w xdyn] = HVCIter(p);    
...
trainingNeuronsSubsong{1}.thres = 12; % criteria for participation during subsong (thres from testLatSig -- must fire at consistent latency more than 12 times in the bout of ~100 syllables to count as participating)
...
trainingNeuronsPsyl{1}.thres = 4; % criteria for participation during subsong (thres from testLatSig -- must fire at consistent latency more than 4 times in the bout of 10 syllables to count as participating)
...
plotAlternating(wProto, xdynProto, p.trainint, trainingNeuronsPsyl, PlottingParams)
set(gca, 'color', 'none')
...
trainingNeuronsAlt{2}.tind = find(mod(1:p.nsteps, 2*p.trainint)==p.trainint+1);
trainingNeuronsAlt{1}.candLat = 1:p.trainint; 
trainingNeuronsAlt{2}.candLat = 1:p.trainint; 
trainingNeuronsAlt{1}.thres = 2; % criteria for participation during subsong (thres from testLatSig -- must fire at consistent latency more than 2 times in the bout of 5 syllables (of each type) to count as participating)
...
set(gca, 'color', 'none');
PlottingParams.axesPosition = [PlottingParams.thisPanel/nplots-2*spacing, bottom, 40*scale, height];
plotAlternating(wSplit1, xdynSplit1, p.trainint, trainingNeuronsAlt, PlottingParams)
set(gca, 'color', 'none')
...
set(gca, 'color', 'none');
PlottingParams.axesPosition = [PlottingParams.thisPanel/nplots-2*spacing, bottom, 40*scale, height];
plotAlternating(wSplit2, xdynSplit2, p.trainint, trainingNeuronsAlt, PlottingParams)
set(gca, 'color', 'none')
...
for t = 1:nsteps
    ...
    dw_hLTDpre = eta*ones(n,1)*max(0, sum(w+dw_STDP,1)-Wmax);  % Weights leaving cells (pre)
    dw_hLTDpost = eta*max(0, sum(w+dw_STDP,2)-Wmax)*ones(1,n);  % Weights onto cells (post)
...
sylIDtoplot = 3; %(don't choose a protosyllable that's at the beginning of a bout)
...
set(gca, 'color', 'none', 'xtick', [0 100], 'xticklabel', {'-100', '0'},'ydir', 'reverse', 'fontsize', numFontSize)
...
hold on; box off
set(gca, 'fontsize', numFontSize)
set(gca, 'color', 'none', 'xtick', 0:50:200,'xticklabel',{'0','','100','','200'}, 'ytick',0:20:100,'ydir', 'reverse',...
