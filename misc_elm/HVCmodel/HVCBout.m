...
addParamValue(p,'beta',0.05);      % global inhibition strength
addParamValue(p,'eta',.05);         % learning rate parameter
addParamValue(p,'epsilon',.15);      % relative strength of heterosynaptic LTD
addParamValue(p,'tau',3);           % time-constant of neural adaptation (only used if alpha is not 0)
...
addParamValue(p,'gamma',.01);        % for splitting, if gamma=1 then neurons will only fire if they are activated more than average compared to other active neurons. 0 = normal rule
...
for i = 1:nsteps
    ...
    inhscaling = 10; r = max(0,w*oldx + b - p.beta*sum(oldx)*(1+inhscaling*sum(w,2)/Wmax)- p.alpha*y); 
    
    temp = sum(r>0); %mean(r(r>0)); % Average of all feedforward activity
