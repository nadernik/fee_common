clear all
pathname = 'Y:\Inscopix\6719_Snapshots\'; 
Folders = dir([pathname 'Examples*'])
%% make compiled 
for folderi = 1:length(Folders); 
    dbasepath = fullfile(pathname, Folders(folderi).name); 
        ...
        if sum(dbase.SegmentIsSelected{filei}==1)>0
            ...
            for bi = 1:size(bouts,1)
                ...
                if length(useframes)>0
                    ...
                    display(['folder ' num2str(folderi) ', SONG file ' num2str(filei) ' bout ' num2str(bi)])
                end
            end
        end
...
for folderi = 1:length(Folders);
    ...
    saveherecnmfe = fullfile(pathname, Folders(folderi).name, 'cnmferesults'); 
    ...
    nam = fullfile(pathname, Folders(folderi).name, 'compiled.mat');
    cnmfe_choose_data;
...
%% plot #neurons as a function of day
N = [];
Dnum = [];
Nsegs = [];
figure; hold on; shg
for folderi = 1:length(Folders);
    ...
    load(fullfile(dbasepath, 'compiled.mat'), 'segs');
    load(saveherecnmfe, 'neuron'); 
    Nsegs(folderi) = size(segs,1);
    N(folderi) = size(neuron.C,1); 
    Dnum(folderi) = datenum(Folders(folderi).name(10:19)); 
    Folders(folderi).name(10:19)
    text(Nsegs(folderi), N(folderi),Folders(folderi).name, 'interpreter', 'none')
    hold on;
end
plot(Nsegs,N, 'k.')
ylabel('# neurons detected'); xlabel('# segs included')
xlim(xlim+[0 diff(xlim)*1.25])
%% plot rasters
...
for folderi = 1:length(Folders);
    ...
    title(num2str(Folders(folderi).name), 'interpreter', 'none');
    ...
    axis tight
    figure(folderi+numel(Folders)); clf
    subplot(2,1,1); hold all
    tmp = reshape(sum(neuron.A,2),300,400);
    imagesc(-tmp, -[prctile(tmp(:),99) prctile(tmp(:),50)]); 
    title(num2str(Folders(folderi).name), 'interpreter', 'none');
    for ni = 1:Nneurons
    ...
    axis image; axis off
    subplot(2,1,2); 
    imagesc(squeeze(max(Y,[],3))); axis image; axis off; colormap gray
    set(gcf, 'papersize', [8.5 11], 'paperposition', [0 0 8.5 11])

    % %
end
%% print all the open figures (and close them in the process)
% for fi = 1:22
%     me = gcf; 
%     print
%     close(me);
