...
clear all; 
XLS = importdata('C:/Users/emackev/Dropbox (MIT)/MackeviciusLabPresentations/DeafenedYoung.xlsx'); % edit isolate_inventory
...
rootSaveHere = '\\feebox6\shared\emackev\AcqGui\DeafenedYoung'; 
for row = 4%2:3
...
for dayi = 1:length(AllDays)
    try
        ...
        allfiles = [Wav; Dat]; 
        if length(allfiles) > 2 % if at least 10 files
...
%% segment song (do this with electro_gui)... or don't...

%% make pages for binder
DumpFigsHere = 'C:\Users\emackev\Documents\MATLAB\code\RasterPlots\DeafenedYoung';
mkdir(DumpFigsHere); 
FilesPerPage = 15; 
SecondsPerFile = 4; 
for row = 4% 2:3;%
...
%         'interpreter', 'none'); 
    ...
    if c == FilesPerPage || fi == length(dbase.SoundFiles)
        ...
        saveas(gcf, fullfile(DumpFigsHere, ...
            ['DeafenedYoung' (birdnum{row}) '_' num2str(d) '.jpg'])); 
        clf
        d = d+1; 
        c = 1;
    end
