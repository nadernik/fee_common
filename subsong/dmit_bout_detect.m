function dmit_bout_detect(pathi,soundchan,pressurechan)

%%% written by Lena Veit

%pathi = 'Z:\Data\subsong_HVC\to1610\2010-04-29_temp\temp_temp'; %%%%% change here!
%soundchan = 0;
%pressurechan = 1;
% pathi = uigetdir('Z:\Data\subsong_HVC\', 'Choose the directory that contains songs');
% soundchan = 0;
% pressurechan = 4;
...
%% Main function
if isempty(dir([pathi filesep 'analysis.mat'])) % no analysis file
    if isempty(dir([pathi filesep 'bouts'])) % no bout folder
        mkdir([pathi filesep 'bouts']); % then make a bout folder
    end

    dbase = [];
    dbase.PathName = [pathi];

    s_files = dir([dbase.PathName filesep '*chan' num2str(soundchan) '.dat']); % file properties of sound channel
    dbase.SoundLoader = 'AA_daq'; % recorded by acquisitionGui
    if ~isempty(pressurechan)
        p_files = dir([dbase.PathName filesep '*chan' num2str(pressurechan) '.dat']); % file properties of pressure channel
        dbase.ChannelLoader = 'AA_daq'; % recorded by acquisitionGui
        % files = dir([dbase.PathName filesep '*.wav']); % recorded by SAP
        % dbase.SoundLoader = 'WaveRead'; % recorded by SAP
    end

    dbase.SoundFiles = s_files;
    if ~isempty(pressurechan)
        dbase.ChannelFiles = {p_files};
    else
        dbase.ChannelFiles = {};
    end    
    ...
    for fl = 1:length(s_files) % for all the files
    ...
    dbase.AnalysisState.EventLims = repmat([1e-3 3e-3],1,1); % -1ms to 3ms in event viewer
    dbase.FileLength = zeros(1,length(s_files));
    dbase.Times = zeros(1,length(s_files));

    fig = figure;
    subplot('position',[.05 0.35 0.9 0.6]);
    text(0,3,[pathi],'fontsize',14,'interpreter','none','horizontalalignment','center');
    hold on
    ...
    % if stopped in the middle last time
    if ~isempty(dir([pathi filesep 'analysis_incomplete.mat']))
        load([pathi filesep 'analysis_incomplete.mat']);
    end

    for fl = dbase.AnalysisState.CurrentFile+1:length(s_files) % for all the files
        ...
        try
            [a fs dateandtime label props] = egl_AA_daq([pathi filesep s_files(fl).name], 1);
            dbase.Times(fl) = dateandtime;
            dbase.FileLength(fl) = length(a);
            if ~isempty(pressurechan)
                [pressdata fs dateandtime label props] = egl_AA_daq([pathi filesep p_files(fl).name], 1);

            end
            % FOR SAP
            %                       [a fs] = wavread([fold(bird).name
            %                       filesep days(dy).name filesep files(fl).name]);
        catch
            disp('fail');
            a = [];
        end
        ...
        if ~isempty(a)
            ...
            params.IsSplit = 0;
            segs = DA_segmenter(amp,fs,th,params); % segment audio file
            ...
            syll = segs(find(sel==1),:); % selected segment
            if ~isempty(syll)
                intr = find(syll(2:end,1)-syll(1:end-1,2)>.5*fs); % bout: sequences of syllables separated by more than 500ms(DA thesis p.49)
                ons = [1; intr+1];
                offs = [intr; size(syll,1)];
                for c = 1:length(ons) % number of bouts
                    temp.files(end+1) = fl;
                    temp.bouts(end+1,:) = [max(1,round(syll(ons(c),1)-.7*fs)) min(length(a),round(syll(offs(c),2)+.7*fs))]; % include +/- 700 ms
                    f = find(segs(:,1)>temp.bouts(end,1) & segs(:,2)<temp.bouts(end,2)); % all segments within bouts
                    ...
                    if ~isempty(pressurechan)
                        prs.Fs = fs;
                        prs.Data = pressdata(temp.bouts(end,1):temp.bouts(end,2)); % extract a bout from pressure signal
                    ...
                    while iss==0
                        try
                            ...
                            if ~isempty(pressurechan)
                                save([dbase.PathName '\bouts\pres' num2str(fl,'%05.f') '_' num2str(c,'%03.f') '.mat'],'prs');
                            end
                            iss = 1;
                        catch
                            disp('fail');
            ...
            while iss==0
                try
                    save([pathi filesep 'analysis_incomplete.mat'],'dbase','temp');
                    iss = 1;
                catch
                    disp('fail');
                    pause(1);
    ...
    while iss==0
        try
            ...
            save([pathi filesep 'analysis.mat'],'dbase');
            delete([pathi filesep 'analysis_incomplete.mat']);
            iss = 1;
        catch
            disp('fail');
            pause(1);
    ...
    dbase = [];
    dbase.PathName = [pathi filesep 'bouts'];
    ...
    while iss==0
        try
            save([pathi filesep 'bouts' filesep 'analysis.mat'],'dbase');
            iss = 1;
        catch
            disp('fail');
            pause(1);
